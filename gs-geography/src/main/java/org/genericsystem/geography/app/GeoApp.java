...
@DependsOnModel({ Subdivision.class, Continent.class, Country.class, City.class, Building.class })
...
public class GeoApp extends RootTagImpl {
	...
	public static class GeoScript implements Script {
		...
		Generic continent, country, adm1, adm2, adm3, city, name;
		Generic continentInstance, countryInstance, adm1Instance, adm2Instance, adm3Instance, cityInstance;
		...
		public void run(Root engine) {
			...
			try {
				...
				while ((line = reader.readLine()) != null) {
					try {
						parts = line.split("\\|");
						System.out.println(
								"adm3" + parts[0] + "@" + parts[1] + "@" + parts[2] + "@" + parts[3] + "@" + parts[4]);
						adm2Instance = adm2.getInstance("ADM2_" + parts[0] + "_" + parts[1] + "_" + parts[2]);
						adm3Instance = adm3.addInstance(
								"ADM2_" + parts[0] + "_" + parts[1] + "_" + parts[2] + "_" + parts[3], adm2Instance);
			...
			// Populated places
			try {
				reader = new BufferedReader(new FileReader("src/main/resources/ppl.csv"));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
