...
public class VicariousThreadLocal<T> extends ThreadLocal<T> {
	...
	/**
	 * Maintains a strong reference to value for each thread, so long as the Thread has not been collected. Note, alive Threads strongly references the WeakReference&lt;Thread> through weakThread.
	 */
	...
	/**
	 * Executes task with thread-local set to the specified value. The state is restored however the task exits. If uninitialised before running the task, it will remain so upon exiting. Setting the thread-local within the task does not affect the state
	 * after exitign.
	 */
	...
	/**
	 * Holds strong reference to a thread-local value. The WeakReference is to a thread-local representing the current thread.
	 */
