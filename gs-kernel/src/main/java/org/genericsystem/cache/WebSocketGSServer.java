...
public class WebSocketGSServer extends AbstractGSServer {
	...
	public WebSocketGSServer(GSDeploymentOptions options) {
		super(options);
	...
	public static void main(String[] args) {
		new WebSocketGSServer(new GSDeploymentOptions()).start();
	}
		...
		for (int i = 0; i < 2 * Runtime.getRuntime().availableProcessors(); i++) {
			...
			httpServer.websocketHandler(webSocket -> {
				...
				webSocket.handler(buffer -> {
					...
					webSocket.writeBinaryMessage(getReplyBuffer(methodId, root, gsBuffer));
				});
