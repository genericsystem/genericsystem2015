...
public class PowerListModel extends Model {

	private final Engine engine;

	private final Property<String> name = new SimpleStringProperty();
	private final Property<Predicate<PowerModel>> mode = new SimpleObjectProperty<>(ALL);

	private final ObservableList<PowerModel> powerModels;// = FXCollections.<CarModel> observableArrayList(CarModel -> new Observable[] { CarModel.getCompleted() });
	private final FilteredList<PowerModel> filtered;// = new FilteredList<>(CarModels);
	...
	private final ObservableBooleanValue hasPowerModel;// = Bindings.lessThan(0, Bindings.size(PowerModels));
	private final ObservableValue<Boolean> hasNoPowerModel;// = Bindings.not(hasCarModel);
	private final ObservableValue<Boolean> allMode = Bindings.equal((ObservableObjectValue<Predicate<PowerModel>>) mode, ALL);
	private final ObservableValue<Boolean> activeMode = Bindings.equal((ObservableObjectValue<Predicate<PowerModel>>) mode, ACTIVE);
	private final ObservableValue<Boolean> completedMode = Bindings.equal((ObservableObjectValue<Predicate<PowerModel>>) mode, COMPLETE);
	private final Property<PowerModel> selection = new SimpleObjectProperty<>();
	...
	public PowerListModel(Engine engine) {
		this.engine = engine;
		ObservableList<PowerModel> instances = new Transformation2<>(engine.find(Car.class).getObservableSubInstances(), g -> new PowerModel(this, g));
		...
		filtered = new FilteredList<>(powerModels);
		filtered.predicateProperty().bind(Bindings.createObjectBinding(() -> mode.getValue(), mode));
		completedCount = Bindings.size(powerModels.filtered(COMPLETE));
		activeCount = Bindings.size(powerModels.filtered(ACTIVE));
		...
		hasCarModel = Bindings.lessThan(0, Bindings.size(powerModels));
		hasNoCarModel = Bindings.not(powerModels);
