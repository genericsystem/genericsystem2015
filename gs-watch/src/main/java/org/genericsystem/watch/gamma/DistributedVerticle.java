...
public class DistributedVerticle extends AbstractVerticle {
	...
	public void start() throws Exception {
		...
		vertx.eventBus().consumer(PRIVATE_ADDRESS, handler -> {
			...
			File file = new File(System.getenv("HOME") + "/copy/" + fileName);
			if (!file.exists()) {
		...
		vertx.eventBus().consumer(PUBLIC_ADDRESS, message -> {
			// System.out.println("Receive on plublic adress private address : " + (String) message.body() + " on : " + PRIVATE_ADDRESS + " " + Thread.currentThread());
			roundrobin.register((String) message.body());
		});
		vertx.setPeriodic(AVAILABILITY_PERIODICITY, h -> {
			// System.out.println("Periodic publish : "+PRIVATE_ADDRESS+" " + Thread.currentThread());
	...
	private <T> void download(Future<T> future, String fileName, String ip) {
		// System.out.println("Execute blocking task : " + Thread.currentThread());
		...
		try {
			File file = new File(System.getenv("HOME") + "/copy/" + fileName);
			new File(System.getenv("HOME") + "/copy/" + fileName.substring(0, fileName.lastIndexOf("/"))).mkdirs();
		...
		// System.out.println("Blocking task complete on thread : " + Thread.currentThread());
		future.complete();
	}
