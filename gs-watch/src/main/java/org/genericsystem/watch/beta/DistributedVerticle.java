...
public class DistributedVerticle extends AbstractVerticle {
	...
	protected static final String IP_ADDRESS = "192.168.1.16";
			...
			try {
				for (Generic task1 : taskType.getInstances()) {
					if (STARTED.equals(new JsonObject((String) task1.getValue()).getString("state"))) {
						System.out.println("PROBLEM : " + ((String) task1.getValue()));
						addMessage(Paths.get(new JsonObject((String) task1.getValue()).getString("file")), new JsonObject((String) task1.getValue()).getInteger("step"), new JsonObject((String) task1.getValue()).getLong("task"), TODO,
				...
				for (Generic message1 : messageType.getInstances()) {
					if (INPROGRESS.equals(new JsonObject((String) message1.getValue()).getString("state"))) {
						...
						addMessage(Paths.get(new JsonObject((String) message1.getValue()).getString("file")), new JsonObject((String) message1.getValue()).getInteger("step"), new JsonObject((String) message1.getValue()).getLong("task"), TODO,
				...
				for (Generic message : messageType.getInstances()) {
					...
					if (TODO.equals(json.getString("state"))) {
						...
						if (workerAddress != null) {
							message.remove();
							Generic inProgress = messageType.addInstance(new JsonObject().put("task", json.getLong("task")).put("state", INPROGRESS).put("max_parallel_executions", json.getInteger("max_parallel_executions"))
									.put("step", json.getLong("step")).put("file", json.getString("file")).put("IP", IP_ADDRESS).encodePrettily());
							cache.flush();
							vertx.eventBus().send(workerAddress, inProgress.getValue(), TIMEOUT, reply -> {
								...
								cache.safeConsum(nothing -> {
									...
									if (reply.failed()) {
										...
										addMessage(Paths.get(js.getString("file")), js.getInteger("step"), js.getLong("task"), TODO, js.getInteger("max_parallel_executions"));
		...
		vertx.eventBus().consumer(PRIVATE_ADDRESS, message -> {
			...
			cache.safeConsum(nothing -> {
				...
				vertx.executeBlocking(future -> {
					...
					client.getNow(port, ip_sender, remoteDirectory + task.getString("file"), resp -> resp.bodyHandler(body -> {
						try {
	...
	private boolean classify(JsonObject task) {
		...
		if (savedFile != null) {
			...
			cache.safeConsum(nothing -> {
				addMessage(savedFile, task.getInteger("step") + 1, task.getLong("task"), TODO, task.getInteger("max_parallel_executions"));
			});
	...
	protected void addMessage(Path file, int step, long timestamp, String state, int parallel_executions) {
		...
		messageType.addInstance(new JsonObject().put("task", timestamp).put("state", state).put("max_parallel_executions", parallel_executions).put("step", step).put("file", file.toString()).encodePrettily());
		cache.flush();
